...
def test_roce():
    ...
    assert return_on_capital_employed(100, 20, 100, 300, 200) == 20.0


def test_roce_invalid():

    assert return_on_capital_employed(100, 20, -500, -500, 0) is None
...
def test_debt_free():
    ...
    assert debt_to_equity(0, 100, 300) == 0.0


def test_negative_equity():

    assert debt_to_equity(200, -500, -100) is None


def test_high_leverage():

    assert high_leverage_flag(6, "Industrials")


def test_financial_not_flagged():

    assert not high_leverage_flag(8, "Financials")
