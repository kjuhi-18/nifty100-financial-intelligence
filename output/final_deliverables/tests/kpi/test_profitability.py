...
def test_operating_profit_margin_crosscheck():
    ...
    result = operating_profit_margin(operating_profit=220, sales=1000, source_opm=25)
...
def test_return_on_equity_normal():
    ...
    assert return_on_equity(net_profit=120, equity_capital=100, reserves=500) == 20.0


def test_return_on_equity_negative_equity():

    assert return_on_equity(net_profit=100, equity_capital=-50, reserves=25) is None
...
def test_return_on_assets_normal():
    ...
    assert return_on_assets(net_profit=150, total_assets=1000) == 15.0
