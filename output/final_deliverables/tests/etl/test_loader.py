...
def test_column_names(tmp_path):
    ...
    df = pd.DataFrame({"company": [1], "sales": [100]})
    ...
    assert list(loaded.columns) == ["company", "sales"]
...
def test_load_all_returns_dict(tmp_path):
    ...
    data = loader.load_all(["a.xlsx", "b.xlsx"])
...
def test_first_dataframe_shape(tmp_path):
    ...
    create_excel(tmp_path, "a.xlsx", pd.DataFrame({"A": [1, 2, 3]}))
...
def test_second_dataframe_shape(tmp_path):
    ...
    create_excel(tmp_path, "b.xlsx", pd.DataFrame({"A": [1], "B": [2]}))
...
def test_loaded_values(tmp_path):
    ...
    loaded = loader.load_excel("values.xlsx", header=0)
