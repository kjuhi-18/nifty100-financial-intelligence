...
def dq04_balance_sheet(bs_df):
    ...
    for _, row in bs_df.iterrows():
        ...
        diff_pct = abs(assets - liabilities) / assets * 100
...
def dq05_opm_validation(pl_df):
    ...
    for _, row in pl_df.iterrows():
        ...
        diff_pct = abs(expected - row["operating_profit"]) / expected * 100
...
def dq07_tax_rate(pl_df):
    ...
    invalid_rows = pl_df[
        (pl_df["tax_percentage"] < 0) | (pl_df["tax_percentage"] > 100)
    ]
...
def dq08_eps_sign(pl_df):
    ...
    invalid_rows = pl_df[(pl_df["net_profit"] > 0) & (pl_df["eps"] <= 0)]
...
def dq09_dividend_cap(pl_df):
    ...
    invalid_rows = pl_df[pl_df["dividend_payout"] > 500]
...
def dq14_coverage_check(df):
    ...
    coverage = df.groupby("company_id")["year"].nunique().reset_index()

    low_coverage = coverage[coverage["year"] < 5]
...
def dq15_null_critical_fields(df, table_name, required_cols):
...
def dq16_duplicate_documents(doc_df):
    ...
    duplicates = doc_df[doc_df.duplicated(subset=["company_id", "Year"], keep=False)]
