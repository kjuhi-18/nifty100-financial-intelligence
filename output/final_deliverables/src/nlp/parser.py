...
class NLPParser:
    ...
    def __init__(self):
        ...
        self.analysis_path = self.project_root / "data" / "raw" / "analysis.xlsx"
        ...
        self.pattern = re.compile(
            r"(\d+)\s*Years?:?\s*([-]?\d+(?:\.\d+)?)%", re.IGNORECASE
        )
    ...
    def load_analysis(self):
        ...
        self.analysis = pd.read_excel(self.analysis_path, skiprows=1)

        print("Companies :", len(self.analysis))
    ...
    def export_outputs(self):
        ...
        self.parsed.to_csv(parsed_file, index=False)

        self.failures.to_csv(failures_file, index=False)
    ...
    def verify_outputs(self):
        ...
        print(self.parsed["metric_type"].value_counts())

        print()

        print("Sample Parsed Records")

        print(self.parsed.head(10))
    ...
    def cross_validate(self):
        ...
        latest = ratios.sort_values("company_id").groupby("company_id").tail(1)
        ...
        for _, row in self.parsed.iterrows():
            ...
            actual = latest.loc[latest["company_id"] == company, metric]
    ...
    def export_review(self):
        ...
        self.review.to_csv(review_file, index=False)
        ...
        print("Manual Review Required :", self.review["manual_review"].sum())
        # ==================================================
    ...
    def day29_summary(self):
        ...
        print(f"analysis_parsed.csv    : {'PASS' if parsed_file.exists() else 'FAIL'}")

        print(
            f"parse_failures.csv     : {'PASS' if failures_file.exists() else 'FAIL'}"
        )

        print(f"analysis_validation.csv: {'PASS' if review_file.exists() else 'FAIL'}")
        ...
        if not self.review.empty:
            ...
            print(f"Manual Review Cases : {self.review['manual_review'].sum()}")
