...
class ProsConsGenerator:
    ...
    def load_data(self):
        ...
        self.cashflow = pd.read_sql("SELECT * FROM cashflow", self.conn)
        ...
        companies = pd.read_sql("SELECT id FROM companies", self.conn)
...
""",
    ...
    def confidence(self, value, threshold):
        ...
        score = min(100, 60 + abs(value - threshold) * 2)
    ...
    def export_output(self):
        ...
        self.pros_cons.to_csv(output_file, index=False)
    ...
    def validate(self):
        ...
        summary = (
            self.pros_cons.groupby(["company_id", "type"]).size().unstack(fill_value=0)
        )
        ...
        summary["PASS"] = (summary["Pro"] >= 1) & (summary["Con"] >= 1)
    ...
    def statistics(self):
        ...
        print(self.pros_cons["rule_id"].value_counts())

        print()

        print("Type Distribution")

        print(self.pros_cons["type"].value_counts())

        print()

        print("Average Confidence")

        print(round(self.pros_cons["confidence_pct"].mean(), 2))
    ...
    def day30_summary(self):
        ...
        print(f"pros_cons_generated.csv : {'PASS' if output_file.exists() else 'FAIL'}")
        ...
        total_pros = len(self.pros_cons[self.pros_cons["type"] == "Pro"])

        total_cons = len(self.pros_cons[self.pros_cons["type"] == "Con"])
        ...
        print(self.pros_cons["rule_id"].value_counts().sort_index())
