...
st.sidebar.success(f"Selected : {selected_year}")
...
avg_roe = round(ratios["return_on_equity_pct"].mean(), 2)

median_de = round(ratios["debt_to_equity"].median(), 2)

median_rev = round(ratios["revenue_cagr_5yr"].median(), 2)

debt_free = ratios["debt_to_equity"].fillna(999).eq(0).sum()

median_pe = None

if "price_to_earnings" in ratios.columns:

    median_pe = round(ratios["price_to_earnings"].median(), 2)

elif "pe_ratio" in ratios.columns:

    median_pe = round(ratios["pe_ratio"].median(), 2)
...
with c5:
    ...
    st.metric("Median Revenue CAGR", median_rev)
...
checks = [len(companies) > 0, len(ratios) > 0]
