...
selected_group = st.selectbox("Peer Group", peer_groups["peer_group_name"])
...
company_metrics = peer_df[peer_df["company_id"] == selected_company]

peer_avg = peer_df.groupby("metric")["value"].mean()

metrics = company_metrics["metric"].tolist()

company_values = company_metrics["value"].fillna(0).tolist()

peer_values = [peer_avg.get(metric, 0) for metric in metrics]
...
fig.add_trace(
    go.Scatterpolar(
        r=peer_values, theta=metrics, line=dict(dash="dash"), name="Peer Average"
    )
)
