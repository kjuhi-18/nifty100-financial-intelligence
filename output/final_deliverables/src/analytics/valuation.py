...
class ValuationEngine:
    ...
    def load_data(self):
        ...
        self.market_df = pd.read_excel(self.market_cap_path)

        self.ratios = pd.read_sql("SELECT * FROM financial_ratios", self.conn)

        self.companies = pd.read_excel(
            self.project_root / "data" / "raw" / "companies.xlsx", skiprows=1
        )
    ...
    def prepare_latest_snapshot(self):
        ...
        market = (
            self.market_df[self.market_df["year"] == latest_year]
            .copy()
        ...
        # Companies
        companies = self.companies.copy().drop(
            columns=["roce_percentage", "roe_percentage"], errors="ignore"
        )

        # Sectors
        sectors = self.sectors.copy().drop(columns=["id"], errors="ignore")

        # Merge everything
        self.latest = (
            market.merge(ratios, on="company_id", how="left")
    ...
    def compute_valuation(self):
        ...
        median_pe = (
            self.market_df.groupby("company_id")["pe_ratio"]
            .median()
            .reset_index()
        ...
        df.loc[df["pe_ratio"] > df["sector_median_pe"] * 1.5, "flag"] = "Caution"

        df.loc[df["pe_ratio"] < df["sector_median_pe"] * 0.7, "flag"] = "Discount"
    ...
    def preview_valuation(self):
        ...
        print(self.valuation["flag"].value_counts())
    ...
    def export_outputs(self):
        ...
        output.to_excel(summary_file, index=False)

        flags = output[output["flag"] != "Fair"].copy()
    ...
    def verify_exports(self):
        ...
        print(self.summary_output["flag"].value_counts())
    ...
    def validate_outputs(self):
        ...
        print(f"valuation_summary.xlsx : {'PASS' if summary_file.exists() else 'FAIL'}")

        print(f"valuation_flags.csv    : {'PASS' if flags_file.exists() else 'FAIL'}")
        ...
        sector_summary = (
            self.valuation.groupby("broad_sector")["sector_median_pe"]
            .first()
            .reset_index()
        )
    ...
    def day26_summary(self):
        ...
        print(
            f"Discount Companies : {(self.summary_output['flag'] == 'Discount').sum()}"
        )

        print(
            f"Caution Companies  : {(self.summary_output['flag'] == 'Caution').sum()}"
        )
