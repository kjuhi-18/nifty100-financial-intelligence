...
def calculate_cagr(begin, end, years):
    ...
    if pd.isna(begin) or pd.isna(end) or years <= 0:
        return None
...
fr.composite_quality_score
    ...
    def get_fcf_cagr(self):
        ...
        df["year_num"] = pd.to_numeric(
            df["year"].str.extract(r"(\d{4})")[0], errors="coerce"
        )
        ...
        for company_id, group in df.groupby("company_id"):
            ...
            if len(group) < 6:
                ...
                rows.append({"company_id": company_id, "fcf_cagr_5yr": None})
            ...
            value = calculate_cagr(
                start["free_cash_flow_cr"], latest["free_cash_flow_cr"], 5
            )

            rows.append({"company_id": company_id, "fcf_cagr_5yr": value})
    ...
    def get_revenue_cagr(self):
        ...
        for company_id, group in df.groupby("company_id"):
            ...
            if len(group) < 6:
                ...
                rows.append({"company_id": company_id, "revenue_cagr_5yr": None})
                continue

            latest = group.iloc[-1]
            start = group.iloc[-6]

            value = calculate_cagr(start["sales"], latest["sales"], 5)

            rows.append({"company_id": company_id, "revenue_cagr_5yr": value})
...
def run_kmeans(df, scaled_df):
    ...
    # Temporary names
    result["cluster_name"] = "Cluster " + result["cluster_id"].astype(str)
...
def profile_clusters(clustered):
    ...
    summary = clustered.groupby("cluster_id")[FEATURES].agg(["mean", "median"]).round(2)
...
def generate_outlier_report(df):
    ...
    for sector, group in data.groupby("broad_sector"):
        ...
        for feature in OUTLIER_FEATURES:
            ...
            flagged = group.loc[
                z_scores.abs() > 3, ["company_id", "company_name", "broad_sector"]
            ].copy()
...
if __name__ == "__main__":
    ...
    latest = latest.merge(revenue, on="company_id", how="left")
    ...
    clustered, model = run_kmeans(latest, scaled)
    ...
    output.to_csv(CLUSTER_FILE, index=False)
