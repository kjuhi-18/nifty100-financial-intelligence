...
class SprintReview:
    ...
    def load_data(self):
        ...
        print(f"Rows Loaded : {len(self.financial_ratios)}")
    ...
    def check_required_columns(self):
        ...
        for column in required:
            ...
            print(f"{column:<35}", "OK" if exists else "MISSING")
    ...
    def screener_preview(self):
        ...
        screener = screener.dropna(subset=["return_on_equity_pct", "debt_to_equity"])
        ...
        screener = screener.sort_values("return_on_equity_pct", ascending=False)

        # ------------------------------------------
        # Save CSV
        # ------------------------------------------

        screener.to_csv(SCREENER_OUTPUT, index=False)
        ...
        available_columns = [
            column for column in preview_columns if column in screener.columns
        ]

        print(screener[available_columns].head(20).to_string(index=False))

        print()

        print(f"Screener saved to:\n{SCREENER_OUTPUT}")
    ...
    def generate_review_report(self):
        ...
        report.append("-" * 40)
        report.append("• ROE = Net Profit / (Equity Capital + Reserves)")
        report.append("• Debt to Equity = Borrowings / Shareholder Equity")
        report.append("• Asset Turnover = Sales / Total Assets")
        report.append("• Interest Coverage = Operating Profit / Interest")
        report.append("• Free Cash Flow = CFO + Investing Activity")
        report.append("• CapEx Intensity = abs(Investing Activity) / Sales × 100")
        report.append("• Revenue, PAT and EPS CAGR computed for 3, 5 and 10 years")
        report.append("")

        report.append("Edge Case Handling")
        report.append("-" * 40)
        report.append("• Financial sector companies excluded from Debt/Equity warning")
        report.append("• CAGR handles ZERO_BASE")
        report.append("• CAGR handles INSUFFICIENT history")
        report.append("• CAGR handles TURNAROUND")
        report.append("• CAGR handles DECLINE_TO_LOSS")
        report.append("• CAGR handles BOTH_NEGATIVE")
        report.append("")

        report.append("ROE / ROCE Review")
        report.append("-" * 40)
        report.append("Computed ratios are compared against companies.xlsx.")
        report.append("Differences greater than 5% are logged.")
        report.append("Each anomaly is categorized as:")
        report.append("  - Data Source Issue")
        report.append("  - Version Difference")
        report.append("  - Formula Discrepancy")
        report.append("")
        ...
        with open(SPRINT_REPORT, "w", encoding="utf-8") as file:

            file.write("\n".join(report))

        print(f"Review saved to:\n{SPRINT_REPORT}")
    ...
    def manual_spot_check(self):
        ...
        available = [c for c in columns if c in self.financial_ratios.columns]
        ...
        for company in companies:
            ...
            df = self.financial_ratios[self.financial_ratios["company_id"] == company][
                available
            ]
...
def main():
    ...
    try:
        ...
        print("✓ financial_ratios table verified")

        print("✓ capital_allocation.csv verified")

        print("✓ ratio_edge_cases.log verified")

        print("✓ screener_preview.csv generated")

        print("✓ sprint2_review.txt generated")

        print("✓ Manual spot check completed")
