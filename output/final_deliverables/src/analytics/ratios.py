...
def operating_profit_margin(
    ...
    computed = percentage(operating_profit, sales)
    ...
    if source_opm is not None:
        ...
        difference = abs(computed - source_opm)
...
def return_on_equity(
    ...
    if denominator <= 0:
        ...
        logger.info("Negative Equity encountered.")

        return None

    return percentage(net_profit, denominator)
...
def return_on_capital_employed(
    ...
    if capital <= 0:
        ...
        logger.info("Invalid Capital Employed.")

        return None

    roce = percentage(ebit, capital)

    if broad_sector is not None and broad_sector.lower() == "financials":

        logger.info("Financial sector company. " "Sector-relative ROCE benchmark.")
...
def calculate_profitability_ratios(row: dict) -> dict:
    ...
    ratios["net_profit_margin_pct"] = net_profit_margin(
        row.get("net_profit", 0), row.get("sales", 0)
    )

    ratios["operating_profit_margin_pct"] = operating_profit_margin(
        row.get("operating_profit", 0), row.get("sales", 0), row.get("opm_percentage")
    )
    ...
    ratios["return_on_capital_employed_pct"] = return_on_capital_employed(
    ...
    ratios["return_on_assets_pct"] = return_on_assets(
        row.get("net_profit", 0), row.get("total_assets", 0)
    )
...
def validate_ratio(
    ...
    if value is None:
        ...
        logger.info(
            f"{ratio_name} unavailable | " f"Company={company_id} | " f"Year={year}"
        )
...
if __name__ == "__main__":
    ...
    results = calculate_profitability_ratios(sample)
...
def debt_to_equity(
    ...
    if equity <= 0:
        ...
        logger.info("Invalid Equity for D/E")
...
def high_leverage_flag(
    debt_equity: float | None, broad_sector: str | None
) -> bool:
    """
    Returns True if
...
def interest_coverage_label(icr: float | None) -> str:
    """
    Display Label
...
def interest_coverage_warning(icr: float | None) -> bool:
    """
    Company may not be
...
def calculate_leverage_ratios(row: dict) -> dict:
    """
    Computes leverage
    and efficiency KPIs
    for a company-year.
    ...
    de = debt_to_equity(
        row.get("borrowings", 0), row.get("equity_capital", 0), row.get("reserves", 0)
    )

    ratios["debt_to_equity"] = de

    ratios["high_leverage_flag"] = high_leverage_flag(de, row.get("broad_sector"))
    ...
    ratios["interest_warning_flag"] = interest_coverage_warning(icr)

    ratios["net_debt"] = net_debt(row.get("borrowings", 0), row.get("investments", 0))

    ratios["asset_turnover"] = asset_turnover(
        row.get("sales", 0), row.get("total_assets", 0)
    )
