...
cashflow_intelligence = pd.read_excel("output/cashflow_intelligence.xlsx")

capital_allocation = pd.read_csv("output/capital_allocation.csv")
expected = cashflow[["company_id", "year"]].drop_duplicates()

actual = capital_allocation[["company_id", "year"]].drop_duplicates()

missing = expected.merge(actual, on=["company_id", "year"], how="left", indicator=True)
...
latest = capital_allocation[capital_allocation["year_num"] == latest_year]

summary = latest.groupby("pattern_label").size().reset_index(name="count")
...
summary.to_csv("output/capital_allocation_summary.csv", index=False)
latest_pattern = (
    capital_allocation.sort_values("year")
    .groupby("company_id")
    .tail(1)[["company_id", "pattern_label"]]
)

cashflow_intelligence = cashflow_intelligence.merge(
    latest_pattern, on="company_id", how="left"
)
...
cashflow_intelligence.to_excel("output/cashflow_intelligence.xlsx", index=False)
changes = []
