...
class PeerEngine:
    ...
    def load_tables(self):
        ...
        self.financial_ratios = pd.read_sql("SELECT * FROM financial_ratios", self.conn)

        self.companies = pd.read_sql("SELECT * FROM companies", self.conn)

        self.peer_groups = pd.read_excel(PEER_GROUP_FILE)
    ...
    def merge_peer_groups(self):
        ...
        self.data = self.data.merge(self.peer_groups, how="left", on="company_id")

        self.data["peer_group_name"] = self.data["peer_group_name"].fillna(
            "No peer group assigned"
        )

        self.data["is_benchmark"] = self.data["is_benchmark"].fillna(False)

        print(self.data.head())

        print()

        print(self.data["peer_group_name"].value_counts())
    ...
    def verify_data(self):
        ...
        print("Unique Companies :", self.data["company_id"].nunique())

        print("Peer Groups :", self.data["peer_group_name"].nunique())

        missing = (self.data["peer_group_name"] == "No peer group assigned").sum()

        print("Companies Without Peer Group :", missing)

        print()

        print(self.data[["company_id", "peer_group_name", "is_benchmark"]].head(15))
    ...
    def compute_peer_percentiles(self):
        ...
        peer_groups = self.data["peer_group_name"].dropna().unique()
        ...
        for group in peer_groups:
            ...
            peer_df = self.data[self.data["peer_group_name"] == group].copy()
            ...
            for metric, inverse in metrics.items():
                ...
                peer_df[f"{metric}_percentile"] = self.percent_rank(
                    peer_df[metric], inverse=inverse
                )
        ...
        print("Percentile Rows :", len(self.peer_percentiles))
    ...
    def populate_peer_percentiles_table(self):
        ...
        self.peer_percentiles.to_sql(
            "peer_percentiles", self.conn, if_exists="append", index=False
        )
    ...
    def verify_database(self):
        ...
        rows = pd.read_sql(query, self.conn).iloc[0, 0]
    ...
    def review_missing_peer_groups(self):
        ...
        missing = self.data[self.data["peer_group_name"] == "No peer group assigned"]

        print(f"Companies : {len(missing)}")

        print()

        print(missing[["company_id", "year"]].to_string(index=False))
    ...
    def peer_group_summary(self):
        ...
        summary = (
            self.data.groupby("peer_group_name")
            .agg(Companies=("company_id", "count"), Benchmarks=("is_benchmark", "sum"))
            .reset_index()
    ...
    def spot_check_peer_groups(self):
        ...
        fmcg = self.peer_percentiles[self.peer_percentiles["peer_group_name"] == "FMCG"]
    ...
    def export_peer_percentiles(self):
        ...
        self.peer_percentiles.to_csv(output_file, index=False)
    ...
    def validate_peer_percentiles(self):
        ...
        print(sorted(self.peer_percentiles["metric"].unique()))

        print()

        print("Peer Groups :", self.peer_percentiles["peer_group_name"].nunique())
    ...
    def final_verification(self):
        ...
        db_rows = pd.read_sql(
            "SELECT COUNT(*) AS total FROM peer_percentiles", self.conn
        ).iloc[0]["total"]
    ...
    def preview_rankings(self):
        ...
        print(self.peer_percentiles.head(20))

        print()

        print(self.peer_percentiles.groupby("metric").size())
    ...
    def prepare_radar_dataset(self):
        ...
        cols = [
            c
            for c in ["company_id", "year", "peer_group_name", "is_benchmark"]
    ...
    def normalise_radar_metrics(self):
        ...
        for metric in self.radar_metrics:
            ...
            values = pd.to_numeric(radar[metric], errors="coerce")
            ...
            radar[metric] = ((values - minimum) / (maximum - minimum)) * 100
    ...
    def preview_radar_data(self):
        ...
        print(self.radar_data[["company_id"] + self.radar_metrics].head(10))
    ...
    def get_peer_average(self, peer_group):
        ...
        peer = self.radar_data[self.radar_data["peer_group_name"] == peer_group]
    ...
    def get_company_metrics(self, company_id):
        ...
        row = self.radar_data[self.radar_data["company_id"] == company_id]
    ...
    def create_radar_chart(self, company_id, peer_group):
        ...
        ax.set_xticklabels(self.radar_labels, fontsize=10)

        ax.set_ylim(0, 100)

        plt.title(f"{company_id}\n{peer_group}", fontsize=14, weight="bold")

        plt.legend(loc="upper right")

        plt.tight_layout()

        filename = RADAR_DIR / f"{company_id}_radar.png"
    ...
    def generate_peer_radar_charts(self):
        ...
        for _, row in self.radar_data.iterrows():
            ...
            self.create_radar_chart(company, peer)
    ...
    def preview_generated_charts(self):
        ...
        charts = sorted(RADAR_DIR.glob("*.png"))

        print(f"Total PNG : {len(charts)}")
    ...
    def get_nifty_average(self):
        ...
        for metric in self.radar_metrics:
            ...
            averages.append(self.radar_data[metric].mean())
    ...
    def create_standalone_chart(self, company_id):
        ...
        nifty_values = [0 if pd.isna(v) else v for v in nifty_values]
        ...
        ax.plot(angles, nifty_values, "--", linewidth=2, label="Nifty 100 Average")
        ...
        plt.title(f"{company_id}\nNo Peer Group", fontsize=14, weight="bold")
    ...
    def generate_standalone_charts(self):
        ...
        for _, row in self.radar_data.iterrows():
            ...
            self.create_standalone_chart(row["company_id"])
    ...
    def verify_radar_charts(self):
        ...
        if len(charts) == len(self.radar_data):
            ...
            print(f"Expected : {len(self.radar_data)}")

            print(f"Generated : {len(charts)}")
    ...
    def radar_summary(self):
        ...
        peer = (self.radar_data["peer_group_name"] != "No peer group assigned").sum()

        standalone = (
            self.radar_data["peer_group_name"] == "No peer group assigned"
        ).sum()
        ...
        print(self.radar_data["peer_group_name"].value_counts().to_string())
    ...
    def benchmark_summary(self):
        ...
        benchmark = self.radar_data[self.radar_data["is_benchmark"] == True]
    ...
    def top_composite_companies(self):
        ...
        print(
            self.radar_data.sort_values("composite_quality_score", ascending=False)[
                cols
            ]
            .head(15)
            .to_string(index=False)
    ...
    def validate_radar_outputs(self):
        ...
        csv_exists = (OUTPUT_DIR / "radar_report.csv").exists()

        print(f"Radar Charts : {len(png_files)}")

        print(f"CSV Report   : {'PASS' if csv_exists else 'FAIL'}")

        print(f"Companies    : {len(self.radar_data)}")
    ...
    def final_day19_verification(self):
        ...
        print(f"Radar Report            : {'PASS' if report_exists else 'FAIL'}")

        benchmark_count = self.radar_data["is_benchmark"].fillna(False).sum()
    ...
    def prepare_peer_comparison(self):
        ...
        peers = pd.read_sql("SELECT * FROM peer_percentiles", conn)
        ...
        latest = latest.merge(
            self.peer_groups[["company_id", "peer_group_name", "is_benchmark"]],
            on="company_id",
            how="left",
        )
        ...
        print(f"Percentile Rows : {len(self.peer_percentiles)}")

        print()

        print(self.peer_report.head())
    ...
    def verify_peer_groups(self):
        ...
        groups = self.peer_report.dropna(subset=["peer_group_name"])[
            "peer_group_name"
        ].unique()
    ...
    def create_peer_workbook(self):
        ...
        for group in groups:
            ...
            self.export_peer_sheet(writer, group)
    ...
    def export_peer_sheet(self, writer, peer_group):

        companies = self.peer_report[
            self.peer_report["peer_group_name"] == peer_group
        ].copy()

        metrics = self.peer_percentiles[
            self.peer_percentiles["peer_group_name"] == peer_group
        ]
        ...
        pivot.columns = [
            c if c == "company_id" else f"{c}_pct_rank" for c in pivot.columns
        ]

        companies = companies.merge(pivot, on="company_id", how="left")

        companies = companies.sort_values("composite_quality_score", ascending=False)

        sheet = peer_group[:31]

        companies.to_excel(writer, sheet_name=sheet, index=False)
    ...
    def format_peer_workbook(self):
        ...
        red_fill = PatternFill(
            fill_type="solid", start_color="F4CCCC", end_color="F4CCCC"
        )
        ...
        header_fill = PatternFill(
            fill_type="solid", start_color="D9EAD3", end_color="D9EAD3"
        )
    ...
    def add_peer_summary(self):
        ...
        summary_fill = PatternFill(
            fill_type="solid", start_color="D0E0E3", end_color="D0E0E3"
        )
    ...
    def validate_peer_workbook(self):
        ...
        expected_groups = self.peer_report.dropna(subset=["peer_group_name"])[
            "peer_group_name"
        ].unique()

        expected_groups = [g for g in expected_groups if g != "No peer group assigned"]
        ...
        for sheet in sheets:
            ...
            print(
                f"{sheet:<25}" f" Rows : {ws.max_row:<4}" f" Columns : {ws.max_column}"
            )
    ...
    def verify_day20_outputs(self):
        ...
        print(f"peer_comparison.xlsx : " f"{'PASS' if workbook.exists() else 'FAIL'}")

        radar = OUTPUT_DIR / "radar_report.csv"

        print(f"radar_report.csv     : " f"{'PASS' if radar.exists() else 'FAIL'}")
        ...
        groups = groups[groups != "No peer group assigned"]
    ...
    def validate_sprint3_outputs(self):
        ...
        print(f"peer_comparison.xlsx      : {'PASS' if peer_file.exists() else 'FAIL'}")

        print(f"radar_charts folder       : {'PASS' if radar_dir.exists() else 'FAIL'}")
        ...
        if screener_file.exists():
            ...
            wb = load_workbook(screener_file, read_only=True)
        ...
        if peer_file.exists():
            ...
            wb = load_workbook(peer_file, read_only=True)
        ...
        charts = list(RADAR_DIR.glob("*.png"))
        ...
        print(f"peer_percentiles rows : {peer_rows}")
    ...
    def verify_quality_compounder(self):
        ...
        screener = pd.read_excel(
            OUTPUT_DIR / "screener_output.xlsx", sheet_name="Quality Compounder"
        )
        ...
        print(screener[required].head(5).to_string(index=False))

        failures = screener[
            (screener["return_on_equity_pct"] <= 15) | (screener["debt_to_equity"] >= 1)
        ]
    ...
    def verify_it_services_peer(self):
        ...
        if not df.empty:
            ...
            print(f"{top.company_id} " f"({top.percentile_rank:.2f}%)")
