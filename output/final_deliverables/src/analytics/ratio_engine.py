...
file_handler = logging.FileHandler(
    OUTPUT_DIR / "ratio_edge_cases.log", mode="w", encoding="utf-8"
)
...
class RatioEngine:
    ...
    def prepare_dataset(self):
        ...
        data = self.pnl.merge(
            self.bs, on=["company_id", "year"], how="left", suffixes=("", "_bs")
        )

        data = data.merge(
            self.cf, on=["company_id", "year"], how="left", suffixes=("", "_cf")
        )
        ...
        data = data.merge(
            self.companies[["id", "book_value", "roe_percentage", "roce_percentage"]],
            left_on="company_id",
            right_on="id",
            how="left",
        )
        ...
        data.drop(
            columns=["id", "id_x", "id_y", "id_bs", "id_cf"],
            inplace=True,
            errors="ignore",
        )
    ...
    def get_company_history(self, company_id):
        ...
        history["norm_year"] = history["year"].apply(
            lambda y: normalize_year(y) or "9999-12"
        )
    ...
    def compute_kpis(self):
        ...
        for _, row in self.data.iterrows():
            ...
            if (
                pd.notna(net_prof)
                and pd.notna(tot_assets)
                and tot_assets > 0
                and tot_assets < net_prof
            ):
                for col in [
                    "equity_capital",
                    "reserves",
            ...
            npm = profitability.get("net_profit_margin_pct")

            cfo_quality = cashflow.get("cfo_quality_label")
            ...
            financial_sector = ["Financials", "Banks", "Banking", "NBFC", "Insurance"]
            ...
            record = {
                ...
                "year": row["year"],
                # Profitability
                "net_profit_margin_pct": profitability.get("net_profit_margin_pct"),
                "operating_profit_margin_pct": profitability.get(
                    "operating_profit_margin_pct"
                ),
                "return_on_equity_pct": profitability.get("return_on_equity_pct"),
                "return_on_capital_employed_pct": profitability.get(
                    "return_on_capital_employed_pct"
                ),
                "return_on_assets_pct": profitability.get("return_on_assets_pct"),
                # Leverage
                "debt_to_equity": leverage.get("debt_to_equity"),
                "interest_coverage": leverage.get("interest_coverage"),
                "asset_turnover": leverage.get("asset_turnover"),
                "high_leverage_flag": leverage.get(
                    "high_leverage_flag"
                ),  # ==================================================
                # Cash Flow KPIs
                # ==================================================
                "free_cash_flow_cr": cashflow.get("free_cash_flow_cr"),
                "capex_cr": cashflow.get("capex_intensity_pct"),
                "cash_from_operations_cr": row.get("operating_activity"),
                "cfo_quality_score": cashflow.get("cfo_quality_score"),
                "capex_intensity": cashflow.get("capex_intensity"),
                "fcf_conversion_rate": cashflow.get("fcf_conversion_rate"),
                "capital_allocation_pattern": cashflow.get(
                ...
                # CAGR Values
                # ==================================================
                "revenue_cagr_3yr": cagr.get("revenue_cagr_3yr"),
                "revenue_cagr_5yr": cagr.get("revenue_cagr_5yr"),
                "revenue_cagr_10yr": cagr.get("revenue_cagr_10yr"),
                "pat_cagr_3yr": cagr.get("pat_cagr_3yr"),
                "pat_cagr_5yr": cagr.get("pat_cagr_5yr"),
                "pat_cagr_10yr": cagr.get("pat_cagr_10yr"),
                "eps_cagr_3yr": cagr.get("eps_cagr_3yr"),
                "eps_cagr_5yr": cagr.get("eps_cagr_5yr"),
                "eps_cagr_10yr": cagr.get("eps_cagr_10yr"),
                # ==================================================
                # CAGR Flags
                # ==================================================
                "revenue_cagr_3yr_flag": cagr.get("revenue_cagr_3yr_flag"),
                "revenue_cagr_5yr_flag": cagr.get("revenue_cagr_5yr_flag"),
                "revenue_cagr_10yr_flag": cagr.get("revenue_cagr_10yr_flag"),
                "pat_cagr_3yr_flag": cagr.get("pat_cagr_3yr_flag"),
                "pat_cagr_5yr_flag": cagr.get("pat_cagr_5yr_flag"),
                "pat_cagr_10yr_flag": cagr.get("pat_cagr_10yr_flag"),
                "eps_cagr_3yr_flag": cagr.get("eps_cagr_3yr_flag"),
                "eps_cagr_5yr_flag": cagr.get("eps_cagr_5yr_flag"),
                "eps_cagr_10yr_flag": cagr.get("eps_cagr_10yr_flag"),
                # ==================================================
                # Composite Score
                # ==================================================
        ...
        print(f"Computed KPI Rows : {len(self.kpi_df)}")

        print()

        print(self.kpi_df.head())
    ...
    def populate_financial_ratios(self):
        ...
        required_columns = [c for c in required_columns if c in self.kpi_df.columns]

        self.cursor.execute("DELETE FROM financial_ratios")
    ...
    def manual_spot_check(self):
        ...
        for company in companies:
            ...
            df = self.kpi_df[self.kpi_df["company_id"] == company][columns]
