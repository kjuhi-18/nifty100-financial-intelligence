...
class ScreenerEngine:
    ...
    def load_tables(self):
        ...
        self.profit_loss = pd.read_sql("SELECT * FROM profitandloss", self.conn)
        ...
        try:
            ...
            self.market_cap = pd.read_sql("SELECT * FROM market_cap", self.conn)
    ...
    def prepare_dataset(self):
        ...
        self.data = self.data.merge(
            self.sectors[["company_id", "broad_sector"]], on="company_id", how="left"
        )
        ...
        if not self.market_cap.empty:
            ...
            if "market_cap" in columns:
                ...
                self.data = self.data.merge(
                    self.market_cap[["company_id", "market_cap"]],
                    on="company_id",
                    how="left",
                )
    ...
    def apply_filter(self, dataframe, column, value, operator):
    ...
    def apply_config_filters(self):
        ...
        if filters["roe_min"]["enabled"]:
            ...
            df = self.apply_filter(
                df, "return_on_equity_pct", filters["roe_min"]["value"], ">="
            )

        if filters["free_cash_flow_min"]["enabled"]:

            df = self.apply_filter(
                df, "free_cash_flow_cr", filters["free_cash_flow_min"]["value"], ">="
            )

        if filters["revenue_cagr_5yr_min"]["enabled"]:

            df = self.apply_filter(
                df, "revenue_cagr_5yr", filters["revenue_cagr_5yr_min"]["value"], ">="
            )

        if filters["pat_cagr_5yr_min"]["enabled"]:

            df = self.apply_filter(
                df, "pat_cagr_5yr", filters["pat_cagr_5yr_min"]["value"], ">="
            )
        ...
        if filters["interest_coverage_min"]["enabled"]:
            ...
            df = self.apply_filter(
                df, "interest_coverage", filters["interest_coverage_min"]["value"], ">="
            )

        if filters["asset_turnover_min"]["enabled"]:

            df = self.apply_filter(
                df, "asset_turnover", filters["asset_turnover_min"]["value"], ">="
            )

        if filters["sales_min"]["enabled"]:

            df = self.apply_filter(df, "sales", filters["sales_min"]["value"], ">=")

        if filters["net_profit_min"]["enabled"]:

            df = self.apply_filter(
                df, "net_profit", filters["net_profit_min"]["value"], ">="
            )

        if filters["eps_cagr_5yr_min"]["enabled"]:

            df = self.apply_filter(
                df, "eps_cagr_5yr", filters["eps_cagr_5yr_min"]["value"], ">="
            )
    ...
    def apply_debt_to_equity_filter(self):
        ...
        self.filtered_data = pd.concat([financials, non_financials], ignore_index=True)

        print(f"Rows after D/E filter : {len(self.filtered_data)}")
    ...
    def apply_interest_coverage_logic(self):
        ...
        df = df[df["interest_coverage"] >= minimum]

        self.filtered_data = df

        print(f"Rows after ICR filter : {len(df)}")
    ...
    def apply_optional_filters(self):
        ...
        for column, config_key, operator in optional_filters:
            ...
            df = self.apply_filter(df, column, filters[config_key]["value"], operator)

        self.filtered_data = df

        print(f"Rows after optional filters : {len(df)}")
    ...
    def normalize_metric(self, series, inverse=False):
    ...
    def generate_metric_scores(self):
        ...
        metrics = {
            "roe_score": ("return_on_equity_pct", False),
            "roce_score": ("return_on_capital_employed_pct", False),
            "npm_score": ("net_profit_margin_pct", False),
            "fcf_score": ("free_cash_flow_cr", False),
            "cfo_score": ("cash_from_operations_cr", False),
            "revenue_score": ("revenue_cagr_5yr", False),
            "pat_score": ("pat_cagr_5yr", False),
            "de_score": ("debt_to_equity", True),
            "icr_score": ("interest_coverage", False),
        }
        ...
        for score_name, (column, inverse) in metrics.items():
            ...
            if column in df.columns:
                ...
                df[score_name] = self.normalize_metric(df[column], inverse=inverse)
        ...
        print(df[[c for c in df.columns if c.endswith("_score")]].head())
    ...
    def compute_composite_score(self):
        ...
        cfo_pat_ratio = df["cash_from_operations_cr"] / df["net_profit"]

        cfo_pat_score = self.normalize_metric(cfo_pat_ratio)

        cash_quality = (
            df["fcf_score"] * 0.15 + cfo_pat_score * 0.10 + fcf_positive * 0.05
        )
        ...
        growth = df["revenue_score"] * 0.10 + df["pat_score"] * 0.10

        # --------------------------------------------------
        # Leverage (15%)
        # --------------------------------------------------

        leverage = df["de_score"] * 0.10 + df["icr_score"] * 0.05
        ...
        df["composite_score"] = df["composite_score"].clip(lower=0, upper=100)

        self.data = df

        print(df[["company_id", "year", "composite_score"]].head())
    ...
    def verify_composite_score(self):
        ...
        print(f"Minimum Score : {self.data['composite_score'].min():.2f}")

        print(f"Maximum Score : {self.data['composite_score'].max():.2f}")

        print(f"Average Score : {self.data['composite_score'].mean():.2f}")

        print("\nTop 10 Composite Scores\n")

        preview = self.data.sort_values("composite_score", ascending=False)[
    ...
    def compute_sector_relative_score(self):
        ...
        for sector in sectors:
            ...
            sector_scores = self.normalize_metric(df.loc[mask, "composite_score"])

            df.loc[mask, "sector_composite_score"] = sector_scores
    ...
    def sector_leaderboard(self):
        ...
        leaderboard = (
            latest.sort_values(
                ["broad_sector", "sector_composite_score"], ascending=[True, False]
            )
        ...
        print(
            leaderboard[
                ["broad_sector", "company_id", "sector_composite_score"]
    ...
    def composite_statistics(self):
        ...
        print(self.data[["composite_score", "sector_composite_score"]].describe())

        print("\nTop 10 Overall\n")

        print(
            self.data.sort_values("composite_score", ascending=False)[
                ["company_id", "composite_score", "sector_composite_score"]
            ]
            .head(10)
            .to_string(index=False)
        )
    ...
    def preset_filter(self, filters):
        ...
        df["year_num"] = (
            df["year"].astype(str).str.extract(r"(\d{4})")[0].fillna(9999).astype(int)
        )
    ...
    def save_preset(self, name):
        ...
        self.filtered_data.to_csv(output, index=False)
    ...
    def turnaround_watch(self):
        ...
        if "revenue_cagr_3yr" in df.columns:
            ...
            df = df[df["revenue_cagr_3yr"] > 10]

        else:

            print("Revenue CAGR 3yr not available - skipping filter")
    ...
    def export_screener_workbook(self):
        ...
        for preset_name, preset in presets.items():
            ...
            available = [c for c in columns if c in self.filtered_data.columns]
            ...
            data = self.filtered_data.sort_values("composite_score", ascending=False)

            for _, row in data.iterrows():

                values = [row[col] for col in available]
    ...
    def verify_screener_output(self):
        ...
        print(f"Minimum : {self.data['composite_score'].min():.2f}")

        print(f"Maximum : {self.data['composite_score'].max():.2f}")

        print(f"Average : {self.data['composite_score'].mean():.2f}")
        ...
        print(f"Minimum : {self.data['sector_composite_score'].min():.2f}")

        print(f"Maximum : {self.data['sector_composite_score'].max():.2f}")

        print(f"Average : {self.data['sector_composite_score'].mean():.2f}")
    ...
    def screener_summary(self):
        ...
        if "year" in latest.columns:
            ...
            latest = latest.sort_values(["company_id", "year"])
        ...
        preview_columns = [c for c in preview_columns if c in latest.columns]

        print(latest[preview_columns].head(10).to_string(index=False))
    ...
    def verify_results(self):
        ...
        print(f"Rows Returned : {len(self.filtered_data)}")

        print(f"Columns : {len(self.filtered_data.columns)}")
        ...
        duplicates = self.filtered_data.duplicated(subset=["company_id", "year"]).sum()
