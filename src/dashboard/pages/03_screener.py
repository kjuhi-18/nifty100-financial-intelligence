...
de_max = st.sidebar.slider(
    ...
    0.0,
    10.0,
    preset_values.get("de", 2.0)
)

fcf_min = st.sidebar.number_input(
    "Minimum Free Cash Flow",
    value=preset_values.get("fcf", 0.0)
)
...
filtered = filtered[
    (filtered["revenue_cagr_5yr"] >= rev_min)
]
...
if "price_to_earnings" in filtered.columns:
    filtered = filtered[
        filtered["price_to_earnings"] <= pe_max
    ]
...
st.caption(
    ...
    f"Exporting {len(filtered)} companies with {len(display_columns)} columns."
