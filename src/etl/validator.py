...
class DataValidator:
    ...
    def load_data(self):
        ...
        self.data_path / "profitandloss.xlsx",
        header=1
        )

        balancesheet = pd.read_excel(
        self.data_path / "balancesheet.xlsx",
        header=1
        )

        cashflow = pd.read_excel(
        self.data_path / "cashflow.xlsx",
        header=1
        )
        ...
        self.data_path / "analysis.xlsx",
        header=1
        )

        documents = pd.read_excel(
        self.data_path / "documents.xlsx",
        header=1
        )

        prosandcons = pd.read_excel(
        self.data_path / "prosandcons.xlsx",
        header=1
        )
        ...
        if "company_id" in profit.columns:
            profit["company_id"] = profit[
            "company_id"
            ].apply(normalize_ticker)

        if "company_id" in balancesheet.columns:
            balancesheet["company_id"] = balancesheet[
            "company_id"
            ].apply(normalize_ticker)
        ...
        if "company_id" in documents.columns:
            documents["company_id"] = documents[
            "company_id"
            ].apply(normalize_ticker)
        ...
        if "year" in profit.columns:
            profit["year"] = profit["year"].apply(
            normalize_year
            )

        if "year" in balancesheet.columns:
            balancesheet["year"] = balancesheet[
            "year"
    ...
    def run_all(self):
        ...
        self.failures.extend(
            dq12_year_format(profit)
        )

        self.failures.extend(
            dq12_year_format(balancesheet)
        )

        self.failures.extend(
            dq12_year_format(cashflow)
        )
        ...
        self.failures.extend(
            dq13_ticker_format(profit)
        )

        self.failures.extend(
            dq13_ticker_format(balancesheet)
        )

        self.failures.extend(
            dq13_ticker_format(cashflow)
        )
