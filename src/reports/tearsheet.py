...
class Tearsheet:
    ...
    # Need at least 2 points to draw a line
        if len(df) < 2:
            ...
            35,
            120,
            "ROE / ROCE trend unavailable (insufficient data)"
        )
        return
...
# ==========================
    ...
    def draw_footer(self):
        ...
        35,
        12,
        "Generated using N100 Financial Analytics Pipeline"
    )

        self.canvas.drawRightString(
        PAGE_WIDTH - 35,
        12,
        f"Page {self.canvas.getPageNumber()}"
    )
class SectorReport(Tearsheet):
        ...
        "D/E"
                ...
                table = Table(
        table_data,
        colWidths=[120,70,70,45,45,45,45]
    )
...
def create_sector_report(sector):
    ...
    pdf = SectorReport(
        str(SECTOR_REPORT_DIR / f"{sector}_report.pdf")
    )
